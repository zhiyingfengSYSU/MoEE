...
from torchvision.datasets import CIFAR10, CIFAR100, STL10, MNIST, FashionMNIST, SVHN
...
class MNISTDataset(MNIST):
    ...
    def __init__(self, domain='mnist', train=True, transform=None, download=True):
        assert domain in self.all_domains, f"Invalid domain: {domain}"
        data_path = os.path.join(DATA_PATHS["mnist"], domain)
        super().__init__(data_path, train=train, transform=transform, download=download)
...
class SVHNDataset(SVHN):
    ...
    def __init__(self, domain='SVHN', split='train', transform=None, download=True):
        assert domain in self.all_domains, f"Invalid domain: {domain}"
        data_path = os.path.join(DATA_PATHS["SVHN"], domain)
...
class STLDataset(STL10):
    ...
    def __init__(self, domain='stl',split='train', transform=None, download=True):
        assert domain in self.all_domains, f"Invalid domain: {domain}"
        data_path = os.path.join(DATA_PATHS["stl"], domain)
        super().__init__(data_path, split='train', transform=transform, download=download)

class FASHION_MNISTDataset(FashionMNIST):
    all_domains = ['fashion_mnist']
    resorted_domains = {
        0: ['fashion_mnist'],
    }
    num_classes = 10  # may not be correct

    def __init__(self, domain='fashion_mnist', train=True, transform=None, download=True):
        assert domain in self.all_domains, f"Invalid domain: {domain}"
        data_path = os.path.join(DATA_PATHS["fashion_mnist"], domain)
        super().__init__(data_path, train=train, transform=transform, download=download)
...
def extract_labels(dataset: Dataset):
    ...
    dl = DataLoader(dataset, batch_size=512, drop_last=False, num_workers=0, shuffle=False)
    labels = []
