...
def train_gen(generator, start_iter, user_class, method, m_in, m_out, glob_iter, user_classifier, oe_batch_size,
              total_iter, state, max_iter, net, train_loader, optimizer, verbose=0):
...
##inversion attack
def inversion_train(generator, start_iter, user_class, method, m_in, m_out, glob_iter, user_classifier, oe_batch_size,
                    total_iter, state, max_iter, net, train_loader, optimizer, verbose=0, logistic_regression=None,
                    weight_energy=None, numclasses=10):
    net.train()  # enter train mode
    generator.eval()
    data_iterator = iter(train_loader)
    generative_alpha = exp_lr_scheduler(glob_iter, decay=0.98, init_lr=0.1)
...
def topk_inversion_train(args, num_features, num_classes, number_dict, data_dict, sample_number, eye_matrix,
                         sample_from, generator, start_iter, user_class, method, m_in, m_out, glob_iter,
                         user_classifier, total_iter, state, max_iter, net, train_loader, optimizer, verbose=0,
                         logistic_regression=None, weight_energy=None, select=None, soft=False, optimizer_fc=None,
    ...
    student_logits = torch.zeros((args.num_classes, args.num_classes))
    student_logits = student_logits.to('cuda:0')
    student_features = torch.zeros((args.num_classes, num_features))
    student_features = student_features.to('cuda:0')
    features_set = {i: [] for i in range(args.num_classes)}
    amount_label = [0 for _ in range(args.num_classes)]
    ...
    for i in tqdm(range(max_iter), disable=verbose < 1):
        ...
        if sum_temp == num_classes * sample_number and total_iter < start_iter:
            ...
            for index in range(len(oody)):
                ...
                data_dict[dict_key] = torch.cat((data_dict[dict_key][1:], oodz[index].detach().view(1, -1)), 0)
            ...
            for index in range(num_classes):
                if index not in user_class:
                    if torch.isnan(temp_precision).any() or torch.isnan(mean_embed_id[index]).any():
                        ood_samples = []
                    else:
                        new_dis = torch.distributions.multivariate_normal.MultivariateNormal(mean_embed_id[index], covariance_matrix=temp_precision)
                        ...
                        if start == False:
                            ...
                            ood_labels = torch.full((select,), index)
                        else:
                            ood_samples = torch.cat((ood_samples, negative_samples[index_prob]), 0)
                            ood_labels = torch.cat((ood_labels, torch.full((select,), index)), 0)
            ...
            if len(ood_samples) != 0:
                ...
                if method == 'energy':
                    Ec_out = -torch.logsumexp(logit_given_gen, dim=1)
                    Ec_in = -torch.logsumexp(x, dim=1)
                    loss += generative_alpha * (torch.pow(F.relu(Ec_in - m_in), 2).mean() + torch.pow(F.relu(m_out - Ec_out), 2).mean())
                elif method == 'OE':
                    loss += generative_alpha * (- (logit_given_gen.mean(1) - torch.logsumexp(logit_given_gen, dim=1)).mean())
                    ...
                    input_for_lr = torch.cat((energy_score_for_fg, energy_score_for_bg), -1)
                    labels_for_lr = torch.cat((torch.ones(len(x)).cuda(), torch.zeros(len(logit_given_gen)).cuda()), -1)
                    ...
                    oodclass = [i for i in range(num_classes) if i not in user_class]
                    oody = np.random.choice(oodclass, select)
                    oody = torch.LongTensor(oody).cuda()
                    ood_samples = generator(oody).clone().detach()
                    logit_given_gen = user_classifier(ood_samples)
                    loss += generative_alpha * F.cross_entropy(logit_given_gen, oody)  # encourage different outputs
                elif method == 'crossentropy2':
                    ood_labels = ood_labels.cuda()
                    logit_given_gen = user_classifier(ood_samples)
                    loss += generative_alpha * F.cross_entropy(logit_given_gen, ood_labels)  # encourage different outputs
        ...
        if i == max_iter - 1:
            for id in range(len(target)):
                features_set[int(target[id])].append(output[id].detach())

            for id in range(len(target)):
                student_logits[target[id]] = student_logits[target[id]] + x[id].detach()
                student_features[target[id]] = student_features[target[id]] + output[id].detach()
                amount_label[target[id]] = amount_label[target[id]] + 1

        # exponential moving average
        loss_avg = loss_avg * 0.8 + float(loss) * 0.2

    if iter == max_iter - 1:
        for ilabel in range(args.num_classes):  # 只需要最后一轮的logits
            student_logits[ilabel] = student_logits[ilabel] / (amount_label[ilabel] + 1e-5)
            student_features[ilabel] = student_features[ilabel] / (amount_label[ilabel] + 1e-5)
    state['train_loss'] = loss_avg
    # for name, p in net.get_fc().named_parameters():
    #   param_norm = torch.norm(p.grad)
    #    print("torch.norm(fc_head.grad)", param_norm)

    return loss_avg, total_iter, student_logits, student_features, features_set, net
...
#                 Ec_in = -torch.logsumexp(x, dim=1)
#                 # loss += generative_alpha * (torch.pow(F.relu(Ec_in - m_in), 2).mean() + torch.pow(F.relu(m_out - Ec_out), 2).mean())
#                 loss += args.loss_weight * (torch.pow(F.relu(Ec_in - m_in), 2).mean() + torch.pow(F.relu(m_out - Ec_out), 2).mean())
#             elif method == 'OE':
#                 # loss += generative_alpha * (- (logit_given_gen.mean(1) - torch.logsumexp(logit_given_gen, dim=1)).mean())
#                 loss += args.loss_weight * (- (logit_given_gen.mean(1) - torch.logsumexp(logit_given_gen, dim=1)).mean())
#             elif method == 'energy_VOS':
#                 energy_score_for_fg = log_sum_exp(weight_energy, x, 1)
#                 # energy_score_for_bg = 1 * torch.logsumexp(predictions_ood[0][:, :-1] / 1, 1)
#                 energy_score_for_bg = log_sum_exp(weight_energy, logit_given_gen, 1)
...
def topk_inversion_train_prox(num_classes, number_dict, data_dict, sample_number, eye_matrix, sample_from, generator,
                              start_iter, user_class, method, m_in, m_out, glob_iter, user_classifier, total_iter,
                              state, max_iter, server_model, net, train_loader, optimizer, verbose=0,
                              logistic_regression=None, weight_energy=None, select=None, soft=False, optimizer_fc=None,
                              optimizer_local=None, mu=1e-3):
    net.train()  # enter train mode
    generator.eval()
...
def plot_tsne(X1, X2, X3):
    ...
    plt.scatter(X_norm1[len1:len1 + len2, 0], X_norm1[len1:len1 + len2, 1], label=name3, alpha=0.8, s=size3, c=color3,
                marker=marker3)
    plt.scatter(X_norm1[len1 + len2:len1 + len2 + len3, 0], X_norm1[len1 + len2:len1 + len2 + len3, 1], label=name0,
                alpha=0.8, s=size0, c=color0,
                marker=marker0)
...
def plot_umap(X1, X2, X3=None):
    ...
    plt.rcParams.update({'font.size': 15})
    # plt.scatter(X_norm1[:len1, 0], X_norm1[:len1, 1], label=name1, alpha=0.8, s=size, c=color, marker=marker)
    plt.scatter(X_norm1[len1:len1 + len2, 0], X_norm1[len1:len1 + len2, 1], label=name2, alpha=0.8, s=size2, c=color2,
                marker=marker2)
    plt.scatter(X_norm1[len1 + len2:, 0], X_norm1[len1 + len2:, 1], label=name0, alpha=0.8, s=size0, c=color0,
                marker=marker0)
    plt.xticks([])
    plt.yticks([])
...
def visualization(user_id, num_classes, number_dict, data_dict, sample_number, eye_matrix, sample_from, generator,
                  start_iter, user_class, user_classifier, total_iter, state, max_iter, net, train_loader, verbose=0,
                  logistic_regression=None, weight_energy=None, select=None, soft=0, external_loader=None,
                  select_ID=1000):
...
def visualization_external(user_id, num_classes, number_dict, data_dict, sample_number, eye_matrix, sample_from,
                           generator, start_iter, user_class, user_classifier, total_iter, state, max_iter, net,
                           train_loader, verbose=0, logistic_regression=None, weight_energy=None, select=None,
                           soft=False, external_loader=None):
    net.eval()  # enter train mode
    generator.eval()
    data_iterator = iter(train_loader)
    exdata_iterator = iter(external_loader)
...
def visualization2(user_id, num_classes, number_dict, data_dict, sample_number, eye_matrix, sample_from, generator,
                   start_iter, user_class, user_classifier, total_iter, state, max_iter, net, train_loader, verbose=0,
                   logistic_regression=None, weight_energy=None, select=None, soft=False, external_loader=None):
    net.eval()  # enter train mode
    generator.eval()
    data_iterator = iter(train_loader)
    # exdata_iterator = iter(external_loader)
...
def VOS_train(user_class, model_name, total_iter, state, max_iter, net, train_loader, num_classes, number_dict,
              sample_number, start_iter, data_dict, eye_matrix, logistic_regression, optimizer,
              loss_weight, weight_energy, sample_from, select, verbose=0):
    num_ID = len(user_class)
    ...
    for i in tqdm(range(max_iter), disable=verbose < 1):
        ...
        if loss_weight != 0:
            ...
            for index in range(num_classes):
                sum_temp += number_dict[index]
            lr_reg_loss = torch.zeros(1).cuda()[0]
            if sum_temp == num_ID * sample_number and total_iter < start_iter:
                # print('case 1')
                # maintaining an ID data queue for each class.
                target_numpy = target.cpu().data.numpy()
                for index in range(len(target)):
            ...
            elif sum_temp == num_ID * sample_number and total_iter >= start_iter:
                # print('case 2')
                target_numpy = target.cpu().data.numpy()
                for index in range(len(target)):
                    dict_key = target_numpy[index]
                    ...
                    if index in user_class:
                        if torch.isnan(temp_precision).any() or torch.isnan(mean_embed_id[index]).any():
                            ...
                            if start == False:
                                ...
                                ood_samples = torch.cat((ood_samples, negative_samples[index_prob]), 0)
                            if torch.isnan(ood_samples).any():
                                ood_samples = []
                if len(ood_samples) != 0:
                    # add some gaussian noise
                    # ood_samples = self.noise(ood_samples)
...
# fedprox_vos
def VOS_train_prox(user_class, model_name, total_iter, state, max_iter, server_model, net, train_loader, num_classes,
                   number_dict, sample_number, start_iter, data_dict, eye_matrix, logistic_regression, optimizer,
                   loss_weight, weight_energy, sample_from, select,
                   verbose=0, mu=1e-3):
    net.train()  # enter train mode
    data_iterator = iter(train_loader)
    # print("total iter", total_iter)
...
def VOS_train1(model_name, total_iter, state, max_iter, net, train_loader, train_loader2, num_classes, number_dict,
               sample_number, start_iter, data_dict, eye_matrix, logistic_regression, optimizer, loss_weight,
               weight_energy, sample_from, select,
...
def VOS_train2(model_name, total_iter, state, max_iter, net, train_loader, train_loader2, num_classes, number_dict,
               sample_number, start_iter, data_dict, eye_matrix, logistic_regression, optimizer, loss_weight,
               weight_energy, sample_from, select,
               verbose=0):
    net.train()  # enter train mode
